...
export function Productos() {
  ...
  const agregarProducto = () => {
    ...
    // Validaciones
    if (nombre === "" || isNaN(categoriaId) || !categoriaSeleccionada || isNaN(precio) || precio <= 0) {
  ...
  const guardarCambiosProducto = (productoEditado) => {
    const nuevosProductos = productos.map(producto => producto.getId === productoEditado.getId ? productoEditado : producto);
    setProductos(nuevosProductos);
    const modal = window.bootstrap.Modal.getInstance(document.getElementById("modalEditarProducto"));
    modal.hide();
  };
          ...
          <tbody>
            {productos.map(producto => (
              <tr key={producto.getId}>
                ...
                <td>
                  <button className="btn btn-primary" onClick={() => iniciarEdicionProducto(producto)}>
                    <FontAwesomeIcon icon={faPen} />
                  </button>
                </td>
                <td>
                  <button className="btn btn-danger" onClick={() => eliminarProducto(producto.getId)}>
        ...
        <form className="needs-validation-productos" noValidate>
          <fieldset className="d-flex flex-column justify-content-center text-center">
            ...
            <div className="mb-3">
              ...
              <div className="invalid-feedback">Por favor ingresa un nombre de producto</div>
            </div>
            <div className="mb-3">
              ...
              <div className="invalid-feedback">Por favor selecciona una categoría de producto</div>
            </div>
            <div className="mb-3">
              <label htmlFor="nuevoPrecioProducto" className="form-label">Precio</label>
              <input type="number" className="form-control" id="nuevoPrecioProducto" min="1" onChange={handlePrecioChange} required />
              <div className="invalid-feedback">Por favor ingresa un precio válido</div>
            </div>
            <button type="button" className="btn btn-info" id="agregarProductoButton" onClick={agregarProducto}>
              Agregar Producto
            </button>
          </fieldset>
        </form>
