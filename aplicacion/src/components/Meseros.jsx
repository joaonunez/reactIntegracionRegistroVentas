...
export function Meseros() {
  ...
  const guardarCambiosMesero = (meseroEditado) => {
    const nuevosMeseros = meseros.map(mesero => mesero.getRut === meseroEditado.getRut ? meseroEditado : mesero);
    setMeseros(nuevosMeseros);
          ...
          <tbody>
            {meseros.map(mesero => (
              <tr key={mesero.getRut}>
                ...
                <td>
                  <button className="btn btn-primary" onClick={() => iniciarEdicionMesero(mesero)}>
                    <FontAwesomeIcon icon={faPen} />
                  </button>
                </td>
                <td>
                  <button className="btn btn-danger" onClick={() => eliminarMesero(mesero.getRut)}>
      ...
      <div className="container-fluid m-auto mt-5 col-12 col-xl-4 shadow rounded border border-info p-3 mb-5">
        <form className="needs-validation-meseros" noValidate>
          <fieldset className="d-flex flex-column justify-content-center text-center">
            ...
            <button type="button" className="btn btn-info col-xl-5 m-auto text-center" id="agregarMeseroButton" onClick={agregarMesero}>
              Agregar Mesero
            </button>
          </fieldset>
        </form>
